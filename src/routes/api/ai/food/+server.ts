...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    if (AI_PROVIDER === 'claude' && CLAUDE_API_KEY) {
      result = await processWithClaude(input, 'food');
    } else if (GEMINI_API_KEY) {
      result = await processWithGemini(input, 'food');
    } else {
...
async function processWithGemini(input: string, type: 'food' | 'exercise') {
  const prompt = type === 'food' ? getFoodPrompt(input) : getExercisePrompt(input);
...
async function processWithClaude(input: string, type: 'food' | 'exercise') {
  const prompt = type === 'food' ? getFoodPrompt(input) : getExercisePrompt(input);
...
function getFoodPrompt(input: string): string {
  return `Parse this food description and return ONLY a JSON object with this exact structure:
{
  "foods": [
    {
      "name": "food name",
      ...
      "mealType": "breakfast" | "lunch" | "dinner" | "snack" (guess based on food type)
    }
  ]
}
...
Estimate calories based on typical serving sizes. If multiple foods are mentioned, include all of them in the array.
